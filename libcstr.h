...
// Copies the source string to the destination string and returns the destination.
char* strCopy(char* dest, const char* src) {
...
// Returns true if the string is a palindrome, ignoring non-alphanumeric characters.
bool isPalindrome(const char* str) {
...
bool isPalindromeIgnoreCase(const char* str) {
    ...
    for (i = 0, j = len - 1; i < j; i++, j--) {
        ...
        if (strToLowercase(str[i]) != strToLowercase(str[j])) {
            return false;
        }
    }
    return true;
...
// Compares two strings lexicographically, ignoring case, and returns an integer result.
int strCompareIntIgnoreCase(const char* str1, const char* str2) {
    while (*str1 && (*str1 == *str2)) {
        str1++;
        str2++;
    }
    return strToLowercase((unsigned char)*str1) - strToLowercase((unsigned char)*str2);
}

// Compares two strings lexicographically, ignoring case, and returns true if they are equal.
bool strCompareBoolIgnoreCase(const char* str1, const char* str2) {
    while (*str1 && (*str1 == *str2)) {
        str1++;
        str2++;
...
// Finds the first occurrence of a character in a string and returns a pointer to it.
char* strFindChar(const char* str, int ch) {
    while (*str != '\0' && *str != ch) {
        str++;
    }
...
// Finds the first occurrence of a substring within a string and returns a pointer to it.
char* strFindString(const char* haystack, const char* needle) {
    if (!*needle) {
...
// Switches the case of each character in the string (uppercase to lowercase and vice versa).
char* strSwapcase(char* str) {
    for (int i = 0; str[i] != '\0'; i++) {
        if (isupper(str[i])) {
            str[i] = tolower(str[i]);
...
char* strReplace(char* str, const char* old_substr, const char* new_substr) {
    ...
    if (position == NULL) return str;

    int len_before = position - str;
    int len_after = strLength(str) - len_before - strLength(old_substr);
...
// Extracts a substring from the given string starting at the specified position and with the specified length.
char* strSubstring(const char* str, int start, int length) {
...
int strHexadecimalToDecimal(const char* hex_str) {
    ...
    for (int i = 0; hex_str[i] != '\0'; i++) {
        result = result * 16 + (isdigit(hex_str[i]) ? hex_str[i] - '0' : toupper(hex_str[i]) - 'A' + 10);
    }
    return result;
}
